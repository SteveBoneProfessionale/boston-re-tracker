"""Match the scanned Rhode Island board-document extractions back to projects.
...
def main():
    results = json.loads(Path("data/ri_scan_results.json").read_text())
    idmap = json.loads(Path("data/ri_scan_idmap.json").read_text())
    c = connect()
    projects = c.execute(
    ...
    for r in results:
        ...
        for f in FIELDS:
            ...
            record(c, p["id"], f, value=value, outcome="resolved",
                   tier="document_confirmed", source_type="scanned_board_document",
                   source_url=None,
                   source_name=f"RI board document (scanned, read as image): {fname}",
